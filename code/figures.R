...
x <- df %>% filter(wave == 0, dplyr::between(cb_I, quantile(cb_I, pct01), quantile(cb_I, 1-pct01))) %>% select("value" = cb_I) %>% mutate(name = "cb_I", value = value / 605, mean = mean(value), med = median(value), pct = "1%")

y <- df %>% filter(wave == 0, dplyr::between(cb_I, quantile(cb_I, pct05), quantile(cb_I, 1-pct05))) %>% select("value" = cb_I) %>% mutate(name = "cb_I", value = value / 605, mean = mean(value), med = median(value), pct = "5%")

z <- df %>% filter(wave == 0, dplyr::between(cb_I, quantile(cb_I, pct10), quantile(cb_I, 1-pct10))) %>% select("value" = cb_I) %>% mutate(name = "cb_I", value = value / 605, mean = mean(value), med = median(value), pct = "10%")
...
x <- df %>% filter(wave == 0, dplyr::between(cb_II, quantile(cb_II, pct01), quantile(cb_II, 1-pct01))) %>% select("value" = cb_II) %>% mutate(name = "cb_II", value = value / 605, mean = mean(value), med = median(value), pct = "1%")

y <- df %>% filter(wave == 0, dplyr::between(cb_II, quantile(cb_II, pct05), quantile(cb_II, 1-pct05))) %>% select("value" = cb_II) %>% mutate(name = "cb_II", value = value / 605, mean = mean(value), med = median(value), pct = "5%")

z <- df %>% filter(wave == 0, dplyr::between(cb_II, quantile(cb_II, pct10), quantile(cb_II, 1-pct10))) %>% select("value" = cb_II) %>% mutate(name = "cb_II", value = value / 605, mean = mean(value), med = median(value), pct = "10%")
...
x <- df %>% filter(wave == 0, dplyr::between(cb_III, quantile(cb_III, pct01), quantile(cb_III, 1-pct01))) %>% select("value" = cb_III) %>% mutate(name = "cb_III", value = value / 605, mean = mean(value), med = median(value), pct = "1%")

y <- df %>% filter(wave == 0, dplyr::between(cb_III, quantile(cb_III, pct05), quantile(cb_III, 1-pct05))) %>% select("value" = cb_III) %>% mutate(name = "cb_III", value = value / 605, mean = mean(value), med = median(value), pct = "5%")

z <- df %>% filter(wave == 0, dplyr::between(cb_III, quantile(cb_III, pct10), quantile(cb_III, 1-pct10))) %>% select("value" = cb_III) %>% mutate(name = "cb_III", value = value / 605, mean = mean(value), med = median(value), pct = "10%")

a <- rbind(x,y,z) %>% mutate(pct = factor(pct, levels = c("1%", "5%", "10%")))

p3 <- ggplot(a, aes(x=value, y=..density.., fill = pct)) + geom_histogram(binwidth = 40, alpha=0.75) + geom_density(alpha = .3) + labs(y = "Model III", x = "") + geom_vline(aes(xintercept=mean), linetype="dashed") + geom_text(aes(0,0,label = paste0("$ ", round(mean, 0))), vjust = -3, hjust = -1, size = 3, check_overlap = T) + scale_color_manual(values=c("#999999", "#E69F00", "#56B4E9"), guide = "none") + scale_fill_manual(values=c("#999999", "#999999", "#999999"), guide = "none") + theme_minimal() + facet_wrap(~ pct, scales = "free") + theme(axis.text.y=element_blank(), axis.text.x=element_text(angle=45,hjust=1), strip.text.x = element_blank())

x <- df %>% filter(wave == 0, dplyr::between(cb_IV, quantile(cb_IV, pct01), quantile(cb_IV, 1-pct01))) %>% select("value" = cb_IV) %>% mutate(name = "cb_IV", value = value / 605, mean = mean(value), med = median(value), pct = "1%")

y <- df %>% filter(wave == 0, dplyr::between(cb_IV, quantile(cb_IV, pct05), quantile(cb_IV, 1-pct05))) %>% select("value" = cb_IV) %>% mutate(name = "cb_IV", value = value / 605, mean = mean(value), med = median(value), pct = "5%")

z <- df %>% filter(wave == 0, dplyr::between(cb_IV, quantile(cb_IV, pct10), quantile(cb_IV, 1-pct10))) %>% select("value" = cb_IV) %>% mutate(name = "cb_IV", value = value / 605, mean = mean(value), med = median(value), pct = "10%")

a <- rbind(x,y,z) %>% mutate(pct = factor(pct, levels = c("1%", "5%", "10%")))

p4 <- ggplot(a, aes(x=value, y=..density.., fill = pct)) + geom_histogram(binwidth = 40, alpha=0.75) + geom_density(alpha = .3) + labs(y = "Model IV", x = "") + geom_vline(aes(xintercept=mean), linetype="dashed") + geom_text(aes(0,0,label = paste0("$ ", round(mean, 0))), vjust = -3, hjust = -1, size = 3, check_overlap = T) + scale_color_manual(values=c("#999999", "#E69F00", "#56B4E9"), guide = "none") + scale_fill_manual(values=c("#999999", "#999999", "#999999"), guide = "none") + theme_minimal() + facet_wrap(~ pct, scales = "free") + theme(axis.text.y=element_blank(), axis.text.x=element_text(angle=45,hjust=1), strip.text.x = element_blank())
