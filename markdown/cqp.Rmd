...
packages <- c("tinytex", "lfe", "lme4", "plm", "bookdown", "gtsummary", "labelled", "grid", "gridExtra", "rstatix", "flextable", "kableExtra", "stargazer", "tidyverse")
...
```{r fig-enrollment, fig.cap="Rates of youth enrollment and inactivity: Bénin and SSA", fig.height=4, fig.width=8}
```
...
<!-- $$\textit{annual net benefits}_i = \frac{\textit{fees paid}_i}{4} - \textit{apprentice allowances}_i - \frac{\textit{total training costs}_j}{N_j}$$ 
 ...
 for each apprentice $i$ in firm $j$, where $N_j$ is the total number of apprentices in firm $j$.-->
...
``` {r, fig-firmsize, fig.width=7,fig.height=4, fig.cap = "Firm size distributions"}
```
...
``` {r fig-costspie, fig.width=7,fig.height=4, fig.cap = "Breakdown of mean annual training costs per apprentice"}
```

``` {r fig-costspie2, fig.width=7,fig.height=4, fig.cap = "Breakdown of mean annual training costs per firm"}
```
