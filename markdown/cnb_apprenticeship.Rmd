...
Training costs: Table \@ref(tab:tbl-netbenefits) estimates that firm training expenditures, including materials and equipment costs, are an order of magnitude smaller than total allowances on a per-apprentice basis. There is no difference between types of apprentices
...
Net benefits: estimated mean benefits per apprentice ranges from `r mean(df$cb_I/605, na.rm = T)` per year to `r mean(df$cb_III/605, na.rm = T)` \$US per year, depending on the model used. Whether our estimate returns positive or negative net benefits hinges on the inclusion of apprentices' productive contributions: Models I and II exclude this factor, resulting in drastically lower estimated benefits. 
...
The various costs of training reported by the MCs are depicted per firm in Figure \@ref(fig:fig-costspie2) in the Appendix.
